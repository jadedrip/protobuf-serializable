...
public class ProtobufInput implements AutoCloseable {
  ...
  public Float readFloat(final long tag) throws IOException {
    ...
    inputStream.read(bytes);
    int asInt = (bytes[0] & 0xFF) | ((int)(bytes[1] & 0xFF) << 8) |
  ...
  public Double readDouble(final long tag) throws IOException {
    ...
    inputStream.read(bytes);
    long asInt = (long)(bytes[0] & 0xFF) | ((long)(bytes[1] & 0xFF) << 8) |
  ...
  public <T> T readClass(Class<T> clz)
  ...
  public <T> void readCollection(Collection<T> collection, long tag,
                                 Class<T> clz)
      throws IOException, InstantiationException, IllegalAccessException {
    while (nextTag() == tag) {
  ...
  public <T> T readClass(long clzTag, Class<T> clz)
    ...
    if (Integer.class.isAssignableFrom(clz) ||
        int.class.isAssignableFrom(clz)) {
      Long zagzig = readVarint(clzTag);
    ...
    if (sz == 0)
      ...
      return readClass(clz);
  }

  private boolean readTag(final int wireType, final long tag)
      throws IOException {

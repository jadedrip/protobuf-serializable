...
public class ProtobufObjectInputStream implements AutoCloseable {
	ObjectInputStream inputStream;
	Long next = null;

	public ProtobufObjectInputStream(final InputStream inputStream) {
		try {
	...
	public <T> T readClass(long clzTag, Class<T> clz)
		...
		if (Number.class.isAssignableFrom(clz) || long.class.isAssignableFrom(clz) || int.class.isAssignableFrom(clz)
				|| short.class.isAssignableFrom(clz)) {
			long zagzig = zagzig(readVarint(clzTag));
			return (T) Long.valueOf(zagzig);
		}
		...
		for (Field field : clz.getDeclaredFields()) {
			...
			if (Map.class.isAssignableFrom(field.getType())) {
				...
				Collection<?> co = (Collection<?>) field.getType().newInstance();
				CollectionType ann = field.getAnnotation(CollectionType.class);
				if (ann != null) {
					readCollection(co, tag, (Class) ann.type());
				} else {
